 ...
 */
avl_t *avl_insert_recursive(avl_t **tree, avl_t **new, int value)
{
	int bfactor;

	if (*tree == NULL)
		return (*new = binary_tree_node(NULL, value));

	if ((*tree)->n > value)
	{
		(*tree)->left = avl_insert_recursive(&(*tree)->left, new, value);
		if ((*tree)->left == NULL)
			return (NULL);
	}
	else if ((*tree)->n < value)
	{
		(*tree)->right = avl_insert_recursive(&(*tree)->right, new, value);
		if ((*tree)->right == NULL)
			return (NULL);
	}
	else
...
avl_t *avl_insert(avl_t **tree, int value)
{
	...
	avl_insert_recursive(tree, &new, value);
	return new;
}
